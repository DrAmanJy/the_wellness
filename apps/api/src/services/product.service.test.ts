...
describe('ProductService', () => {
  ...
  describe('Read & Delete Operations', () => {
    ...
    it('throws NotFoundError for non-existent product slug', async () => {
      const { NotFoundError } = await import('@wellness/utils');
    ...
    it('throws NotFoundError for getting soft-deleted product by slug', async () => {
      ...
      await expect(productService.getProductBySlug('deleted-slug'))
        .rejects.toThrow(NotFoundError);
    });

    it('returns structured DTO for product details', async () => {
      const p = await factories.createProduct({ slug: 'full-dto', status: 'active' });
    ...
    it('throws ConflictError for duplicate slug on update', async () => {
      ...
      await expect(productService.updateProduct(p.id, { slug: 'existing-slug' }, 'user-1'))
        .rejects.toThrow();
    });
    ...
    it('rolls back product update if category assignment fails', async () => {
      ...
      const updatePromise = productService.updateProduct(p.id, {
        name: 'New Name',
    ...
    it('throws NotFoundError for repeated deletion of soft-deleted product', async () => {
      ...
      await expect(productService.deleteProduct(p.id))
        .rejects.toThrow(NotFoundError);
    });
    ...
    it('supports cursor pagination for public catalog with distinct pages and correct DTO', async () => {
      ...
      if (page1.nextCursor) {
        const parsed = JSON.parse(Buffer.from(page1.nextCursor, 'base64').toString('utf8'));
    ...
    it('filters out soft-deleted variants and categories when fetching product', async () => {
      ...
      const { db, categories, productCategories, productVariants } = await import('@wellness/db');
      
      // Assign categories
      await db.insert(productCategories).values([
        { productId: p.id, categoryId: c1.id, isPrimary: true },
        { productId: p.id, categoryId: c2.id, isPrimary: false }
      ]);
      ...
      await db.update(categories).set({ deletedAt: new Date() }).where(require('drizzle-orm').eq(categories.id, c2.id));
      await db.update(productVariants).set({ deletedAt: new Date() }).where(require('drizzle-orm').eq(productVariants.id, v2.id));
  ...
  describe('Variant CRUD Operations', () => {
    it('throws NotFoundError when adding a variant to a non-existent or soft-deleted product', async () => {
      ...
      await expect(productService.addVariant('00000000-0000-0000-0000-000000000000', { name: 'V1', sku: 'V1', price: '10' }))
        .rejects.toThrow(NotFoundError);
      ...
      await expect(productService.addVariant(p.id, { name: 'V2', sku: 'V2', price: '20' }))
        .rejects.toThrow(NotFoundError);
    });
    ...
    it('throws NotFoundError when updating a soft-deleted variant', async () => {
      ...
      await expect(productService.updateVariant(p.id, variant.id, { price: '15' }))
        .rejects.toThrow(NotFoundError);
    });
    ...
    it('throws NotFoundError when updating a variant that does not belong to the product', async () => {
      ...
      const variant = await productService.addVariant(p1.id, { name: 'V1', sku: 'V1', price: '10' });

      // Attempt to update p1's variant using p2's ID
      await expect(productService.updateVariant(p2.id, variant.id, { price: '15' }))
        .rejects.toThrow(NotFoundError);
    });
  });
  ...
  describe('Image CRUD Operations', () => {
    it('throws NotFoundError when adding an image to a non-existent or soft-deleted product', async () => {
      const { NotFoundError } = await import('@wellness/utils');
      // Non-existent product
      await expect(productService.addProductImage('00000000-0000-0000-0000-000000000000', { url: 'http://example.com/1.jpg', sortOrder: 0 }))
        .rejects.toThrow(NotFoundError);
      ...
      await expect(productService.addProductImage(p.id, { url: 'http://example.com/2.jpg', sortOrder: 0 }))
        .rejects.toThrow(NotFoundError);
    });
    ...
    it('returns images sorted by sortOrder and handles duplicate orders', async () => {
      ...
      await productService.addProductImage(p.id, { url: 'http://example.com/2.jpg', sortOrder: 2, altText: 'Second' });
      await productService.addProductImage(p.id, { url: 'http://example.com/1.jpg', sortOrder: 1, altText: 'First' });
      await productService.addProductImage(p.id, { url: 'http://example.com/2-dup.jpg', sortOrder: 2, altText: 'Duplicate' });
      ...
      expect(found.images[1].sortOrder).toBeGreaterThanOrEqual(found.images[0].sortOrder);
      expect(found.images[2].sortOrder).toBeGreaterThanOrEqual(found.images[1].sortOrder);
    });
    ...
    it('throws NotFoundError when deleting an image that does not belong to the product', async () => {
      ...
      const p1 = await factories.createProduct();
      const p2 = await factories.createProduct();
      const image = await productService.addProductImage(p1.id, { url: 'http://example.com/1.jpg', sortOrder: 0 });

      // Attempt to delete p1's image using p2's ID
      await expect(productService.deleteProductImage(p2.id, image.id))
        .rejects.toThrow(NotFoundError);
    });
  });
});
