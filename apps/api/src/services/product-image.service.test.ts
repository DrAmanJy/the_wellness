...
describe('ProductService - Images', () => {
  ...
  beforeEach(async () => {
    ...
    product = await factories.createProduct({
      createdBy: user.id,
      updatedBy: user.id,
      status: 'active',
    });
  ...
  describe('addProductImage', () => {
    ...
    it('rejects concurrent assignments of multiple primary images via database constraint', async () => {
      ...
      const p1_query = db
        ...
        .set({ isPrimary: true })
        .where(eq(productImages.id, img1.id));
      const p2_query = db
        .update(productImages)
        .set({ isPrimary: true })
        .where(eq(productImages.id, img2.id));
      ...
      if (rejected[0]?.status === 'rejected') {
        const err: { code?: string; cause?: { code?: string }; name?: string; message?: string } =
          rejected[0].reason as never;
        const isConstraint =
          err.code === '23505' ||
  ...
  describe('updateProductImage', () => {
    ...
    it('throws NotFoundError for invalid image ID', async () => {
      await expect(
        productService.updateProductImage(product.id, '00000000-0000-0000-0000-000000000000', {
          isPrimary: true,
        }),
      ).rejects.toThrow(NotFoundError);
    });
  ...
  describe('deleteProductImage', () => {
    ...
    it('throws NotFoundError if image belongs to a different product (IDOR) on delete', async () => {
      ...
      await expect(productService.deleteProductImage(product.id, img.id)).rejects.toThrow(
        NotFoundError,
      );
    });
  });
